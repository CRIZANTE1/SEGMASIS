...
def load_aggregated_data():
    ...
    for i, unit in enumerate(all_units):
        unit_name, unit_id = unit.get('nome_unidade'), unit.get('id')
        ...
        try:
            ...
            data_map = {
                ...
                "company_docs": data.get('company_docs')
            }
    ...
    with st.form("user_form"):
        ...
        if st.form_submit_button("Salvar"):
            ...
            if is_edit_mode:
                ...
                if matrix_manager_global.update_user(user_data['id'], updates):
...
def confirm_delete_dialog(user_id, user_email):
    ...
    if col2.button("Sim, Remover", type="primary", use_container_width=True):
        matrix_manager_global = GlobalMatrixManager()
        if matrix_manager_global.remove_user(user_id):
        ...
        with tab_global_manage:
            ...
            if not all_users_df.empty:
                ...
                if not users_to_delete.empty:
                    user_id = users_to_delete.iloc[0]['id']
                    user_email = users_to_delete.iloc[0]['email']
                    confirm_delete_dialog(user_id, user_email)
            else:
